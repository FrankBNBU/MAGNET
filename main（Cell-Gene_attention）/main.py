...
def build_clarifyGAE_pytorch(data, hyperparams = None):
    ...
    gene_data = data[4]
    print(f"动态检测到的细胞数量: {num_cells1}, 特征数量: {num_cellfeatures1}")
    print(f"基因数据y尺寸: {gene_data.y.size()}")
...
def generate_reconstructed_matrix(model, data, output_path):
    ...
    with torch.no_grad():
        ...
        if hasattr(model, 'decoder') and hasattr(model.decoder, 'forward_all'):
            ...
            print(f"重构邻接矩阵形状: {adj_reconstructed_np.shape}")
            ...
            print(f"重构基因邻接矩阵形状: {gene_adj_reconstructed_np.shape}")
            ...
            print(f"已保存重构细胞邻接矩阵到: {output_cell_path}")
            print(f"已保存重构基因邻接矩阵到: {output_gene_path}")
            
            return adj_reconstructed_np, gene_adj_reconstructed_np
        else:
            print("错误: 模型没有decoder.forward_all方法")
            return None, None
